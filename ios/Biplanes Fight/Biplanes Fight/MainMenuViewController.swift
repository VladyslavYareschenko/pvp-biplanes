...
final class MainMenuViewController: UIViewController {
    ...
    private func buildUI() {
        ...
        botButton.addTarget(
            self,
            action: #selector(didTapBot),
            for: .touchUpInside
        )
        onlineButton.addTarget(
            self,
            action: #selector(didTapOnline),
            for: .touchUpInside
        )
        ...
        [titleLabel, botButton, networkRow, onlineButton].forEach {
    ...
    @objc private func didTapOnline() {
        let host =
            hostField.text?.trimmingCharacters(in: .whitespaces) ?? "127.0.0.1"
